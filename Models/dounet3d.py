    ...
    def forward(self, x):
        ...
        for i, down in enumerate(self.down_path):
            if i>0 and self.do<1:
                x = F.dropout3d(x, p=self.do, training=True)
            x = down(x)
...
class UNetUpBlock(nn.Module):
    ...
    def embed_layer(layer, target_size):
        ...
        diff_x_0 = (target_size[2] - layer_width) // 2
        diff_x_1 = target_size[2] - layer_width - diff_x_0
        layer = F.pad(layer, (diff_x_0, diff_x_1, 0, 0, 0, 0), mode='circular')
        diff_y_0 = (target_size[1] - layer_height) // 2
        diff_y_1 = target_size[1] - layer_height - diff_y_0
        layer = F.pad(layer, (0, 0, diff_y_0, diff_y_1, 0, 0), mode='circular')
        diff_z_0 = (target_size[0] - layer_depth) // 2
        diff_z_1 = target_size[0] - layer_depth - diff_z_0
        layer = F.pad(layer, (0, 0, 0, 0, diff_z_0, diff_z_1))
        return layer
